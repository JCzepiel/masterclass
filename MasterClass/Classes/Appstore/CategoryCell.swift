    ...
    var app: App? {
        didSet {
            if let app = app {
                ...
                if let name = app.Name, let category = app.Category {
                    nameLabel.text = name
                    
                    let rect = NSString(string: name).boundingRect(with: CGSize(width: frame.width, height: 1000), options: [.usesFontLeading, .usesLineFragmentOrigin], attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 14)], context: nil)
                    if rect.height > 20 {
                ...
                if let hasImage = app.ImageName {
                    imageView.image = UIImage(named: hasImage)
                }
            }
        }
    }
