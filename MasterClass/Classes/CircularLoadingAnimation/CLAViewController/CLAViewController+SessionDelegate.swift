...
extension CLAViewController: URLSessionDownloadDelegate {
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        do {
            ...
            DispatchQueue.main.async {
                ...
                if let imageDownload = self.imageDownloader.currentDownloads[originalUrl], let cell = self.tableView.cellForRow(at: imageDownload.indexPath) as? CLAImageCell {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        ...
        if let imageDownload = imageDownloader.currentDownloads[originalImageUrl] {
            ...
            DispatchQueue.main.async {
                if let cell = self.tableView.cellForRow(at: imageDownload.indexPath) as? CLAImageCell {
