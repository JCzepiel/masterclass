...
class CLAImageCell: UITableViewCell {
    var postImageView = UIImageView()
    let circularProgress = CLACircularProgressView()
    ...
    func configure(with downloader: CLAImageDownloader, andUrl url: String, forIndexPath indexPath: IndexPath) {
        if downloader.downloadFinished(for: url) {
            if let image = downloader.finishedDownloads[url] {
