...
protocol SwipeMatchFirestoreLoginControllerDelegate {
...
class LoginController: UIViewController {
    ...
    let passwordTextField: CustomTextField = {
        ...
        //tf.isSecureTextEntry = true //disabling  for now as it could cause a bug
        tf.addTarget(self, action: #selector(handleTextChanged), for: .editingChanged)
        return tf
    }()
    ...
    @objc fileprivate func handleBack() {
        if let hasNavigationController = navigationController, hasNavigationController.viewControllers.count > 1 {
            hasNavigationController.popViewController(animated: true)
        } else {
    ...
    fileprivate func setupBindables() {
        loginViewModel.isFormValid.bind { [unowned self] isFormValid in
            ...
            self.loginButton.backgroundColor = isFormValid ? .red : .lightGray
